...
static void daemonize(void) {
    ...
    * 1.在普通进程中调用fork函数
    * 2.使父进程退出，子进程继续运行
    * 3.子进程变成孤儿进程，被init进程收养
    * 4.子进程是父进程的完全复制，从而实现守护化*/

	if (0 != fork()) exit(0);

    /* 脱离控制终端、进程组和登录会话：
    * 1.使用setsid()设置新会话的首进程
    * 进程组ID就该组进程组长的ID，组长进程存在与否并不关系到进程组的存在与否 
    * 登录会话就是包含一个或者多个进程组的集合，控制终端通常是创建第一个进程的登录终端
    * 控制终端、登录会话和进程组通常是从父进程继承下来的*/

	if (-1 == setsid()) exit(0);
    
    /* 防止孙子进程意外结束，忽略SIFHUP信号，孙子进程会继承这一属性*/
	signal(SIGHUP, SIG_IGN);

    /* 由于会话首进程打开一个尚未与任何会话关联的终端设备时，
    *  此设备自动作为控制终端分配给该会话，与守护进程守护进程不关联控制终端不符,
    *  所以，创建孙子进程，不在是会话后进程从而解决这个问题*/
	if (0 != fork()) exit(0);

    /* 改变当前的工作目录，避免卸载不了文件系统*/
	if (0 != chdir("/")) exit(0);
    /* 可以在这重设文件掩码，防止某些属性被父进程屏蔽
    *  或者重定向标准输入，输出，错误流，因为守护进程没有控制终端
    *  或者关闭文件描述符*/
